<script>
    ...
    // set default prop arguments
    export let yLabel = "count";
    ...
    */
	...
	$: filtered = regex ? graphData.filter(datagroup => regex.test(datagroup.name)) : graphData;

	$: points = filtered.reduce((points, datagroup) => {
		return points.concat(datagroup.data.map(d => ({
			x: d.x,
			y: d.y,
			datagroup
		})));
...
<div class="chart">
	<Pancake.Chart {x1} {x2} y1={y1} y2={y2}>
		...
		<Pancake.Svg>
			{#each filtered as datagroup}
				<Pancake.SvgLine data={datagroup.data} let:d>
					<path class="data" {d}></path>
				</Pancake.SvgLine>
			{/each}

			{#if closest}
				<Pancake.SvgLine data={closest.datagroup.data} let:d>
		...
		{#if closest}
			<Pancake.Point x={closest.x} y={closest.y}>
				...
				<div class="annotation" style="transform: translate(-{100 * ((closest.x - x1) / (x2 - x1))}%,0)">
					<strong>{closest.datagroup.name}</strong>
					<span>{closest.x}: {closest.y} {yLabel}</span>
				</div>
			</Pancake.Point>
		{/if}
